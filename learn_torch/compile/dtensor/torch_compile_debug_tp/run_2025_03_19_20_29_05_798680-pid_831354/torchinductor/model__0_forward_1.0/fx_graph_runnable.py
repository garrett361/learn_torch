...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, primals_2, primals_3):
        device_put = torch.ops.prims.device_put.default(primals_1, device(type='cuda', index=3));  primals_1 = None
...
def load_args(reader):
    ...
    buf1 = reader.storage(None, 1024, device=device(type='cuda', index=3))
    reader.tensor(buf1, (8, 32), is_leaf=True)  # primals_2
    buf2 = reader.storage(None, 1024, device=device(type='cuda', index=3))
