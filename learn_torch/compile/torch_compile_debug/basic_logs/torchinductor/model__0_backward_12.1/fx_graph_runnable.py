...
def load_args(reader):
    ...
    buf1 = reader.storage(None, 256, device=device(type='cuda', index=0), dtype_hint=torch.bool)
    reader.tensor(buf1, (1, 256), dtype=torch.bool, is_leaf=True)  # le
    buf2 = reader.storage(None, 1024, device=device(type='cuda', index=0))
    reader.tensor(buf2, (1, 256), is_leaf=True)  # tangents_1
