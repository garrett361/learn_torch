...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_2, le, permute_3, tangents_1):
        full_default = torch.ops.aten.full.default([], 0.0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=1), pin_memory = False)
...
def load_args(reader):
    buf0 = reader.storage(None, 512, device=device(type='cuda', index=1))
    reader.tensor(buf0, (1, 128), is_leaf=True)  # primals_2
    buf1 = reader.storage(None, 128, device=device(type='cuda', index=1), dtype_hint=torch.bool)
    reader.tensor(buf1, (1, 128), dtype=torch.bool, is_leaf=True)  # le
    buf2 = reader.storage(None, 65536, device=device(type='cuda', index=1))
    reader.tensor(buf2, (128, 128), is_leaf=True)  # permute_3
    buf3 = reader.storage(None, 512, device=device(type='cuda', index=1))
